...
function OutletCard({ outlet, index }) {
  return (
    <motion.div
      ...
      className="glass border border-gray-200 dark:border-white/8 rounded-3xl p-6 card-hover group relative overflow-hidden"
    >
      ...
      <div className="flex items-start gap-4 mb-5 relative">
        ...
        <div>
          <div className="inline-block px-2.5 py-0.5 rounded-full bg-primary-600/20 border border-primary-600/30 text-primary-600 dark:text-primary-400 text-xs font-semibold mb-2">
            {outlet.district}
          </div>
          <h3 className="text-gray-900 dark:text-white font-bold text-base leading-tight group-hover:text-gold-500 dark:group-hover:text-gold-400 transition-colors duration-300">
            {outlet.name}
          </h3>
        </div>
      </div>
      ...
      <div className="space-y-3 mb-6 relative">
        <div className="flex gap-3">
          ...
          <p className="text-gray-500 dark:text-white/60 text-sm leading-relaxed">{outlet.address}</p>
        </div>
        <div className="flex gap-3 items-center">
          <Phone size={15} className="text-primary-500 shrink-0" />
          <a href={`tel:${outlet.phone}`} className="text-gray-500 dark:text-white/60 text-sm hover:text-gold-500 dark:hover:text-gold-400 transition-colors">
            {outlet.phone}
          </a>
        </div>
        <div className="flex gap-3 items-center">
          <Clock size={15} className="text-primary-500 shrink-0" />
          <p className="text-gray-500 dark:text-white/60 text-sm">{outlet.hours}</p>
        </div>
      </div>

      <div className="flex gap-3 relative">
        <a
          href={outlet.mapsUrl} target="_blank" rel="noopener noreferrer"
          className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl bg-primary-600/20 border border-primary-600/30 text-primary-600 dark:text-primary-400 text-sm font-semibold hover:bg-primary-600 hover:text-white transition-all duration-300 hover:border-primary-600"
        >
          <ExternalLink size={14} /> Peta
        </a>
        <a
          href={`https://wa.me/${outlet.whatsapp}`} target="_blank" rel="noopener noreferrer"
          className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl bg-emerald-600/20 border border-emerald-600/30 text-emerald-600 dark:text-emerald-400 text-sm font-semibold hover:bg-emerald-600 hover:text-white transition-all duration-300 hover:border-emerald-600"
        >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full glass-warm border border-primary-600/30 mb-4">
            ...
            <span className="text-primary-600 dark:text-primary-400 text-sm font-semibold tracking-wider">LOKASI KAMI</span>
          </div>
        ...
        <motion.div
          ...
          className="mt-10 glass border border-gray-200 dark:border-white/8 rounded-3xl overflow-hidden"
        >
          <div className="p-4 border-b border-gray-200 dark:border-white/8 flex items-center gap-2">
